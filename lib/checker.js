...
module.exports = class Checker {
  ...
  checkLeanMessage() {
    return new Promise( (resolve, reject) => {
      this.realtime.createIMClient(`${REGION}-alice`).then( alice => {
        return alice.createConversation({
          members: [`${REGION}-bob`],
          name: 'Alice & Bob',
          unique: true,
        }).then( conversation => {
          return this.realtime.createIMClient(`${REGION}-bob`).then( bob => {
